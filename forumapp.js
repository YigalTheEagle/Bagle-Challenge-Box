...
app.get('/gethint',(req,res)=>{
	if(req.headers['x-powered-by-bamba']=='VGhpcyBpcyBqdXN0IHRoZSBBUEkgdG9rZW4=')
	{
		...
		{
			res.send("baglelook={toString:function(){alert(String.fromCharCode(76,111,111,107,32,99,108,111,115,101,108,121,32,111,110,32,116,104,101,32,111,110,99,108,105,99,107,32,118,97,108,117,101,32,111,102,32,116,104,101,32,112,111,115,116,32,99,111,109,109,101,110,116,32,98,117,116,116,111,110,32,45,32,119,104,97,116,32,112,97,114,116,32,105,110,32,116,104,97,116,32,74,97,118,97,83,99,114,105,112,116,32,97,99,116,117,97,108,108,121,32,115,101,110,100,115,32,116,104,101,32,112,111,115,116,32,114,101,113,117,101,115,116,63,13,10,76,111,111,107,32,97,116,32,104,111,119,32,116,104,101,32,118,97,114,32,75,111,98,105,32,105,115,32,117,115,101,100,44,32,97,110,100,32,104,111,119,32,116,104,101,32,82,101,113,117,101,115,116,74,83,79,78,32,97,116,32,116,104,101,32,115,101,114,118,101,114,32,115,105,100,101,32,105,115,32,117,115,101,100,32,115,105,109,105,108,97,114,108,121,46,13,10,85,110,100,101,114,115,116,97,110,100,32,104,111,119,32,116,111,32,105,110,118,111,107,101,32,97,32,102,117,110,99,116,105,111,110,32,105,110,32,74,97,118,97,83,99,114,105,112,116,32,119,105,116,104,111,117,116,32,117,115,105,110,103,32,73,73,70,69,32,102,114,111,109,32,116,104,101,32,99,108,105,101,110,116,32,115,105,100,101,32,99,111,100,101,44,32,97,110,100,32,117,115,101,32,105,116,46))}}");
		}
		else
		{
...
app.get('/gamba-apropo-start',(req,res)=>{
	...
	{
		res.send('bagleapropo="xhr.setRequestHeader(\'IgNoRe-mE\',\'d293IHlvdSBrbm93IGJhc2U2ND8NCmMzUnZjQ0JvWlhKbElIQnNaV0Z6WlEwS1VqSTVkbHBEUW5GaU1raw==\')";');
	}
})
...
app.get('/authenticatedjs',(req,res)=>{
	if(req.headers['x-powered-by-bamba']=='VGhpcyBpcyBqdXN0IHRoZSBBUEkgdG9rZW4=')
	{
	res.write("bagledocument.getElementsByTagName('h2')[0].innerHTML='Stage 2 - Find how to serialize functions!';document.getElementsByTagName('h3')[0].innerHTML='Authenticated! Play close attention to the client side code, it is the only way.';document.getElementsByTagName('input')[0].name='postername';document.getElementsByTagName('input')[1].outerHTML='<textarea name=\x22comment\x22 placeholder=\x22Enter comment here...\x22>';document.getElementsByTagName('input')[0].value='';document.getElementsByTagName('input')[0].placeholder='Enter name';document.getElementsByTagName('button')[0].innerHTML='Post Comment';document.getElementsByTagName('button')[0].setAttribute('onclick','send+\"comment\"');dambo=document.getElementById('sumo');cinderella=document.getElementById('avocado');pomba=document.createElement('table');pomba.setAttribute('align','center');pomba.setAttribute('border','black');dambo.appendChild(pomba);have={toString:function(){var t=SetXHR('GET','/forumapi',apropo);t.send(),t.onreadystatechange=function(){if (4==this.readyState&&200==this.status){var t=JSON.parse(this.responseText);for(i in t.Name){if((parseInt(i)+1)!=t.Name.length){timon[i].tds[1].innerHTML=t.Name[i],timon[i].tds[2].innerHTML=t.Comment[i];}}}}}};apropa='GET';aprapo='/forumapi';var simba=function(){var t=SetXHR(apropa,aprapo,apropo);t.send(),t.onreadystatechange=function(){if(4==this.readyState&&200==this.status){var t=JSON.parse(this.responseText);for(i in t.Name){if((parseInt(i)+1)!=t.Name.length){for(timon[i]={},timon[i].tds=[],timon[i].tr=document.createElement('tr'),j=0;j<3;j++)timon[i].tds[j]=document.createElement('td'),timon[i].tr.appendChild(timon[i].tds[j]);timon[i].tds[0].innerHTML=parseInt(i)+1;timon[i].tds[1].innerHTML=t.Name[i],timon[i].tds[2].innerHTML=t.Comment[i]}else{for (timon[i] = {}, timon[i].tds = [], timon[i].tr = document.createElement('tr'), j = 0; j < 3; j++) timon[i].tds[j] = document.createElement('th'), timon[i].tr.appendChild(timon[i].tds[j]);timon[i].tds[0].innerHTML='Number';timon[i].tds[1].innerHTML='Name';timon[i].tds[2].innerHTML='Comment'};}for(var z=timon.length-1;z>=0;z--){pomba.appendChild(timon[z].tr);}}}}()");
	res.send();
	}
	else
	{
